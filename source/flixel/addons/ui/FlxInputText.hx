...
/**
 * Ne_Eo's modifications:
 * - Fixed bugs where the mouse checks didn't account for the zoom of the camera
 * - Optimized drawcalls
 * 
 * Sword352's modifications:
 * - Clipboard past support (CTRL + V)
 */
...
class FlxInputText extends FlxText
{
	...
	private function onKeyDown(e:KeyboardEvent):Void
	{
		if (hasFocus)
		{
			...
			{
				var clipboardData:String = Clipboard.generalClipboard.getData(TEXT_FORMAT);
				text = text.substring(0, caretIndex) + clipboardData + text.substring(caretIndex);
                caretIndex += clipboardData.length;
				onChange("CLIPBOARD_PAST");
				return;
			}
	...
	private override function calcFrame(RunOnCpp:Bool = false):Void
	{
		...
		if (backgroundSprite != null)
		{
			if (background)
			{
				...
				//backgroundSprite.makeGraphic(1, 1, backgroundColor, false, bgKey);
				//backgroundSprite.setGraphicSize(width, Std.int(height));
		...
		/*if (fieldBorderSprite != null)
		{
			if (fieldBorderThickness > 0)
			{
				...
				fieldBorderSprite.makeGraphic(1, 1, fieldBorderColor, false, key);
				fieldBorderSprite.setGraphicSize(fWidth, fHeight);
		...
		if (backgroundSprite != null)
		{
			if (background)
			{
				...
				backgroundSprite.makeGraphic(1, 1, backgroundColor, false, bgKey);
				backgroundSprite.setGraphicSize(Std.int(width), Std.int(height));
				backgroundSprite.updateHitbox();

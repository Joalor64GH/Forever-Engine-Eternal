...
/**
 * Forever Engine Eternal modifications:
 * - Crash Handler
 * - Function to get the correct alpha
 */
/**
...
class FlxRuntimeShader extends FlxShader {
	...
	+ "
		...
		float flixel_alpha(sampler2D bitmap, vec2 coord)
		{
			return flixel_texture2D(bitmap, coord).a;
		}
	";
	#else
	// No additional data.
	...
	private var compiledShader:Bool = false;
	...
	public function new(fragmentSource:String = null, vertexSource:String = null, glslVersion:Int = 120):Void {
		...
		try {
			if (fragmentSource == null) {
				print('Loading default fragment source...');
				glFragmentSource = processFragmentSource(DEFAULT_FRAGMENT_SOURCE);
			}
			else {
				print('Loading fragment source from argument...');
				glFragmentSource = processFragmentSource(fragmentSource);
			}
			compiledShader = true;
		}
		catch (e) {
			print('Tried to load shader fragment source, which failed with error: ${e} - ${e.details()}', ERROR);
		}

		try {
			if (vertexSource == null) {
				print('Loading default vertex source...');
			...
			print('Tried to load shader vertex source, which failed with error: ${e} - ${e.details()}', ERROR);
		}
	 ...
	 */
	static function processFragmentSource(input:String):String {
	 ...
	 */
	static function processVertexSource(input:String):String {
	...
	/**
	 * Returns the default fragment source
	 */
	public inline static function getDefaultFragment():String
		return processFragmentSource(DEFAULT_FRAGMENT_SOURCE);

	/**
	 * Returns the default fragment source
	 */
	public inline static function getDefaultVertex():String
		return processFragmentSource(DEFAULT_FRAGMENT_SOURCE);
	...
	private function thisHasField(name:String):Null<Bool> {
		if (!compiledShader)
			return null;
	...
	public function setFloat(name:String, value:Float):Void {
		...
		@:privateAccess
		if (prop == null) {
			print('Shader float property ${name} not found.', WARNING);
			return;
		}
		prop.value = [value];
	}
	...
	public function setInt(name:String, value:Int):Void {
		...
		if (prop == null) {
			print('Shader int property ${name} not found.', WARNING);
			return;
		}
		prop.value = [value];
	}
	...
	public function setBool(name:String, value:Bool):Void {
		...
		if (prop == null) {
			print('Shader bool property ${name} not found.', WARNING);
			return;
		}
		prop.value = [value];
	}
	...
	public function setSampler2D(name:String, value:BitmapData) {
		...
		if (prop == null) {
			print('Shader sampler2D property ${name} not found.', ERROR);
			return;
		}
		prop.input = value;
	}

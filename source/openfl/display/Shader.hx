	...
	**/
	...
	@:noCompletion private function __createGLShader(source:String, type:Int):GLShader {
		...
		if (hasInfoLog || compileStatus == 0) {
			var compiledVertex:Bool = type == gl.VERTEX_SHADER;
			var message = (compileStatus == 0) ? "Error" : "Info";
			message += compiledVertex ? " compiling vertex shader" : " compiling fragment shader";
			message += "\n" + shaderInfoLog;
			message += "\n" + source;
			compiledVertex ? caughtVertexException = source : caughtFragmentException = source;

			if (compileStatus == 0) {
				print(message, ERROR);

				/*@:privateAccess
					FNFGame._instance.goToExceptionState('ERROR COMPILING SHADER', 'File: ${compiledVertex ? 'VERTEX' : 'FRAGMENT'}'); */
			}
			else if (hasInfoLog)
				print(message, DEBUG);
		}
	...
	private var caughtFragmentException:String = null;
	private var caughtVertexException:String = null;
	...
	@:noCompletion private function __createGLProgram(vertexSource:String, fragmentSource:String):GLProgram {
		...
		if (gl.getProgramParameter(program, gl.LINK_STATUS) == 0) {
			...
			@:privateAccess
			{
				var shaderLog = Main.game.getLogPath();
				Main.game.writeLog(shaderLog, '${message}\n\nFragment Source:\n${caughtFragmentException}\n\nVertex Source:\n${caughtVertexException}');
				Lib.application.window.alert('Unable to initialize the shader program\nSaved shader log to ${shaderLog}\nPress OK to go back to the game.',
				    "Forever Engine: Eternal - Exception Report");

				glFragmentSource = FlxRuntimeShader.getDefaultFragment();
				glVertexSource = FlxRuntimeShader.getDefaultVertex();
				this.program = null;
			}
		}

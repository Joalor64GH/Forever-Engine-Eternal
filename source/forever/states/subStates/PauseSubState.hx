...
class PauseSubState extends MusicBeatSubState {
	...
	var menuItems:Array<String> = ['Resume', 'Restart Song', 'Go To Options', 'Exit to menu'];
	...
	public function new(x:Float, y:Float) {
		...
		levelInfo.text += Tools.dashToSpace(PlayState.current.curSong);
		...
		levelDifficulty.text += forever.backend.LevelData.curDifficulties[PlayState.storyDifficulty];
	...
	override function update(elapsed:Float) {
		...
		if (controls.pressed("accept")) {
			...
			switch (daSelected) {
				...
				case "Go To Options":
					Main.switchState(new forever.settings.OptionsState(true));
				case "Exit to menu":

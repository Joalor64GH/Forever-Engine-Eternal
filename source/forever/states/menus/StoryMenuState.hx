...
class StoryMenuState extends MusicBeatState {
	...
	override function create() {
		...
		Discord.changePresence('STORY MENU', 'In Menus');
		...
		sprDifficulty = new FlxSprite(leftArrow.x + 130, leftArrow.y).loadGraphic(Paths.image("menus/base/storymenu/difficulties/easy"));
		changeDifficulty();
		...
		mouseHandler.onWheelScroll = changeWeek;
	}
	...
	override function update(elapsed:Float) {
		...
		// scoreText.setFormat(Paths.font("vcr"), 32);
		var lerpVal = Tools.boundFPS(0.5);
		...
		if (!movedBack) {
			if (!selectedWeek) {
				if (controls.anyJustPressed(["up", "down"]))
					changeWeek(controls.justPressed("down") ? 1 : -1);

				rightArrow.animation.play(controls.pressed("right") ? 'press' : 'idle');
				leftArrow.animation.play(controls.pressed("left") ? 'press' : 'idle');

				if (controls.anyJustPressed(["left", "right"]))
					changeDifficulty(controls.justPressed("left") ? -1 : 1);
			}
	...
	function selectWeek() {
		if (weekUnlocked[curWeek]) {
			...
			var diffic:String = LevelData.curDifficulties[curDifficulty].toLowerCase();
			...
			new FlxTimer().start(1, function(tmr:FlxTimer) Main.switchState(new PlayState(PlayState.storyPlaylist[0].toLowerCase(), diffic)));
		}
	}
	...
	function changeDifficulty(change:Int = 0):Void {
		...
		if (curDifficulty < 0)
			curDifficulty = LevelData.curDifficulties.length - 1;
		if (curDifficulty > LevelData.curDifficulties.length - 1)
			curDifficulty = 0;
	...
	function updateDifficultyGraphic() {
		var difficultyGraphic = Paths.image("menus/base/storymenu/difficulties/" + LevelData.curDifficulties[curDifficulty].toLowerCase());
		if (difficultyGraphic != sprDifficulty.graphic) {
			sprDifficulty.loadGraphic(difficultyGraphic);

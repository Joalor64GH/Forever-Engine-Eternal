...
class Alphabet extends FlxSpriteGroup {
	...
	public var xTo = 100;
	public var customLerp:Null<Float> = null;
			...
			if (AlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1 || isNumber || isSymbol) {
				...
				if (FlxG.random.bool(soundChance) || lastPlayed > 2) {
					if (playSounds) {
						...
						var daSound:String = beginPath + soundChoices[cur] + '.ogg';
	...
	override function update(elapsed:Float) {
		if (isMenuItem) {
			...
			y = FlxMath.lerp(y, (scaledY * 120) + (FlxG.height * 0.48), getLerp(elapsed));
			// lmao
			if (!disableX)
				x = FlxMath.lerp(x, (targetY * 20) + 90, getLerp(elapsed));
			else
				x = FlxMath.lerp(x, xTo, getLerp(elapsed));
		}
	...
	function getLerp(elapsed:Float):Float {
		return customLerp != null && customLerp != 0 ? customLerp : elapsed * 6;
	}
}
